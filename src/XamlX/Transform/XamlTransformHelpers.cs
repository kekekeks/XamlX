    ...
    public static class XamlTransformHelpers
    {
            ...
            IXamlAstValueNode node, IXamlType type, out IXamlAstValueNode rv)
        {
            ...
            if (node is XamlAstTextNode tn)
            {
                ...
                {
                    var resolvedType = XamlTypeReferenceResolver.ResolveType(context, tn.Text, tn, true);
                    rv = new XamlTypeExtensionNode(tn, new XamlAstClrTypeReference(tn, resolvedType), type);
                    return true;
                }
