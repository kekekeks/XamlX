...
namespace XamlX.Transform
{
    public class XamlXTransformerConfiguration
    {
        ...
        public bool TryGetCorrectlyTypedValue(IXamlXAstValueNode node, IXamlXType type, out IXamlXAstValueNode rv)
        {
            ...
            if (type.IsEnum && node is XamlXAstTextNode etn)
            {
            ...
            if (TypeMappings.TypeDescriptorContext != null)
            {
                ...
                if (typeConverterAttribute != null)
                {
                    ...
                    var converterType = (arg as IXamlXType) ?? (arg is String sarg ? TypeSystem.FindType(sarg) : null);
                                ...
                                new XamlXStaticOrTargetedReturnMethodCallNode(node, converterMethod,
                                    new[]
                                    {
                                        new XamlXAstNewClrObjectNode(node,
                                            new XamlXAstClrTypeReference(node, converterType),
                                            new List<IXamlXAstValueNode>()),
                                        new XamlXAstContextLocalNode(node, TypeMappings.TypeDescriptorContext),
